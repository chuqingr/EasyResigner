...
//                        validProfiles.append(profile)
                        filteringIdenticalCertificates(profile, &nameIdArr, &validProfiles)
                    } else {
    ...
    private func filteringIdenticalCertificates(_ profile:Profile,_ nameIdArr:inout [String],_ validProfiles:inout [Profile] ) {
        let nameAndId = profile.name + profile.teamIdentifiers.first!
        if nameIdArr.contains(nameAndId) {
            let index = nameIdArr.firstIndex(of: nameAndId)
            let inProfile = validProfiles[index!]
            let inExpiration = inProfile.expirationDate
            if inExpiration!.timeIntervalSinceNow < profile.expirationDate.timeIntervalSinceNow {
                validProfiles.remove(at: index!)
                validProfiles.append(profile)
                nameIdArr.append(nameAndId)
            }
        }else {
            validProfiles.append(profile)
            nameIdArr.append(nameAndId)
